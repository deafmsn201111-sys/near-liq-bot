...
from config import (
    ...
    BINANCE_WS_URL, BINANCE_SYMBOL,
    BYBIT_WS_URL, BYBIT_TOPIC,
    HYPERLIQUID_WS_URL, HYPERLIQUID_COIN,
...
# ─── HTTP Keep-Alive Server (для UptimeRobot) ───────────────
...
def start_http_server():
    ...
    logger.info(f"HTTP keep-alive сервер на порту {HTTP_PORT}")
...
def format_liq_msg(exchange, side, price, qty, value_usd, extra=""):
    s = side.upper().strip()
    if s in ("SELL", "S"):
        emoji, pos = "🔴", "Лонг"
    elif s in ("BUY", "B"):
    ...
    msg = (f"{emoji} <b>Крупная ликвидация NEAR</b>\n\n"
           f"🏦 <b>Биржа:</b> {exchange}\n"
           f"📊 <b>Позиция:</b> {pos}\n"
           f"💰 <b>Объём:</b> ${value_usd:,.2f}\n"
           f"📈 <b>Кол-во:</b> {qty:,.2f} NEAR\n"
           f"💵 <b>Цена:</b> ${price:,.4f}\n"
           f"⏰ <b>Время:</b> {now}")
    if extra:
        msg += f"\n{extra}"
...
class BinanceMonitor(BaseMonitor):
    ...
    def on_open(self, ws):
        logger.info(f"[{self.name}] ✅ Подключено ({BINANCE_SYMBOL})")
        self._delay = 1
    def on_message(self, ws, message):
        try:
            ...
            logger.info(f"[{self.name}] {symbol} {side} {qty:.2f} @ {price:.4f} → ${value:,.2f}")
            if value >= MIN_LIQ_USD:
                send_telegram(format_liq_msg("Binance", side, price, qty, value))
        except Exception as e:
...
class BybitMonitor(BaseMonitor):
    ...
    def url(self):
        return BYBIT_WS_URL
    def on_open(self, ws):
        logger.info(f"[{self.name}] ✅ Подключено, подписка на {BYBIT_TOPIC}")
        ws.send(json.dumps({"op": "subscribe", "args": [BYBIT_TOPIC]}))
        self._delay = 1
    def on_message(self, ws, message):
        try:
            data = json.loads(message)
            ...
            for item in items:
                ...
                value = size * price
                logger.info(f"[{self.name}] {symbol} {side} {size:.2f} @ {price:.4f} → ${value:,.2f}")
                if value >= MIN_LIQ_USD:
                    send_telegram(format_liq_msg("Bybit", side, price, size, value))
        except Exception as e:
            logger.error(f"[{self.name}] Ошибка: {e} | {message[:120]}")
...
class HyperliquidMonitor(BaseMonitor):
    ...
    def on_open(self, ws):
        logger.info(f"[{self.name}] ✅ Подключено, подписка на trades ({HYPERLIQUID_COIN})")
        ws.send(json.dumps({"method": "subscribe", "subscription": {"type": "trades", "coin": HYPERLIQUID_COIN}}))
        self._delay = 1
    def on_message(self, ws, message):
        try:
            data = json.loads(message)
            ...
            for t in trades:
                ...
                logger.info(f"[{self.name}] LIQ {coin} {side} {sz:.2f} @ {px:.4f} → ${value:,.2f} (user={liq_user})")
                if value >= MIN_LIQ_USD:
                    extra = f"🏷 <b>Адрес:</b> <code>{liq_user}</code>"
                    send_telegram(format_liq_msg("Hyperliquid", side, px, sz, value, extra))
        except Exception as e:
            logger.error(f"[{self.name}] Ошибка: {e} | {message[:120]}")
